...
console.log(orders)
  ...
  >
      ...
      {order.orderDetails?.products?.map((product) => (
        <li key={product.id} className="flex items-center p-4 bg-white rounded-lg border border-gray-200">
          ...
          <div className="flex-grow">
            <span className="font-medium text-gray-800">{product.name || 'Producto desconocido'}</span>
            <div className="text-sm text-gray-600">
              ${product.price} x {order.orderDetails?.quantity || 1}
            </div>
          </div>
        </li>
      ))}
    ...
    <p className="mt-4 text-lg font-bold text-gray-900">
      💰 Total: ${order.orderDetails?.price || 0}
    </p>

    <p className="text-gray-700 flex items-center space-x-2 p-2 border-b">
      Contacta al vendedor:  
      <strong className="ml-2">{order.user?.phone || 'No disponible'}</strong>
